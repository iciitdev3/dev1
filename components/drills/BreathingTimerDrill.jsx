...
export default function BreathingTimerDrill({ drill, onComplete, onCancel }) {
  ...
  const getPhaseInstruction = () => {
    switch (phase) {
      case 'ready':
        return t('getComfortable');
      case 'inhale':
        return t('breatheInSlowly');
      case 'hold':
        return t('holdBreathGently');
      case 'exhale':
        return t('breatheOutSlowly');
      default:
        return '';
    }
  };
    ...
    return (
      <Card className="max-w-2xl mx-auto border-0 shadow-xl">
        <CardContent className="p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('wellDone')}</h2>
          <p className="text-gray-600 mb-4">
            {t('breathingCompleteMessage')}
          </p>
          <div className="bg-blue-50 rounded-lg p-4 mb-6">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-[#6495ED]">{cycle}</p>
                <p className="text-sm text-gray-600">{t('breathingCycles')}</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-600">90s</p>
                <p className="text-sm text-gray-600">{t('focusTime')}</p>
              </div>
            </div>
          </div>
        </CardContent>
    ...
    <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center space-y-6">
              ...
              {phase !== 'ready' && (
                <div className="mt-4 max-w-sm mx-auto">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Phase {phaseTime + 1} of {breathingPattern[phase]} {t('seconds')}
                  </p>
                </div>
              )}
            </div>
              ...
              >
                {isRunning ? (
                  ...
                  <>
                    ...
                    <span>{timeLeft === 90 ? t('start') : t('resume')}</span>
                  </>
                )}
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 max-w-sm mx-auto">
              <div className="text-center p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-600">{t('breathingCycles')}</p>
              </div>
              <div className="text-center p-3 bg-gray-50 rounded-lg">
                <p className="text-lg font-bold text-green-600">{Math.round(cycleProgress)}%</p>
                <p className="text-xs text-gray-600">Session {t('progress')}</p>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-0 shadow-lg bg-blue-50">
        <CardContent className="p-4">
          <h3 className="font-medium text-gray-900 mb-2">{t('breathingInstructions')}</h3>
          <div className="space-y-1 text-sm text-gray-700">
            <p>• {t('inhaleInstruction')}</p>
            <p>• {t('holdInstruction')}</p>
            <p>• {t('exhaleInstruction')}</p>
